...
export const SubCategoryPage = () => {
    ...
    const handleEdit = (sub: any) => {
        ...
        setForm({ nombre: sub.nombre, id_categoria: sub.id_categoria || sub.categoria?.id_categoria });
        setIsModalOpen(true);
    };
                ...
                <table className="w-full text-left">
                    <thead className="bg-slate-50 border-b">
                        <tr>
                            ...
                            <th className="px-6 py-4 text-xs font-bold text-slate-500 uppercase">Categoría Padre</th>
                    ...
                    <tbody className="divide-y divide-slate-100">
                        {loading ? (
                            <tr><td colSpan={4} className="text-center py-6 text-slate-400">Cargando...</td></tr>
                        ) : (
                            subCategories.map((sub) => (
                                <tr key={sub.id_subcategoria} className="hover:bg-slate-50/50">
                                    ...
                                    <td className="px-6 py-4 text-sm text-slate-600">{sub.categoria?.nombre}</td>
                                    <td className="px-6 py-4">
                                        <span className="text-[10px] bg-primary-50 text-primary-600 px-2 py-0.5 rounded font-bold uppercase">
                                            {sub.categoria?.area?.nombre}
                                        </span>
                                    </td>
                                    <td className="px-6 py-4">
                                        <div className="flex items-center justify-center space-x-2">
                                            <button onClick={() => handleEdit(sub)} className="p-2 hover:bg-amber-50 text-amber-500 rounded-lg">
                                                <Edit size={16} />
                                            </button>
                        ...
                        <form onSubmit={handleSubmit} className="p-6 space-y-4">
                            ...
                            <div>
                                ...
                                <select className="input-field bg-white text-sm" value={form.id_categoria} onChange={(e) => setForm({ ...form, id_categoria: Number(e.target.value) })}>
                                    ...
                                    {categories.map(c => <option key={c.id_categoria} value={c.id_categoria}>{c.nombre} ({c.area?.nombre})</option>)}
                                </select>
                            </div>
